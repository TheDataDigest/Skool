---
title: "Skool Community Names"
title-block-banner: "#f0f3f5"
title-block-banner-color: "black"
subtitle: "Analyzing 3706 communitiy names by search terms (2024-08-22)" 
author: 
  name: "Torsten Blass"
...
I searched the skool.com discovery page for the following 44 search terms that relate to a **group** in a wider sense:
...
**Academy** is the most common group name with 735 communities.
![42 group search terms and the number of communities for this analysis"](images/search_table.png)
...
It shows that we have 1331 communties that provided a price point for their monthly subscription. The median price for paid communities is \$48, the average (mean) price is \$106.82.
...
The average (mean) free community has `r search_unique_hit %>% filter(access2 == "Free") %>% pull(members_n) %>% mean() %>% round(.,1)` members, which is 476 more than the paid communities average of `r search_unique_hit %>% filter(access2 != "Free") %>% pull(members_n) %>% mean() %>% round(.,1)`.
...
## Some group names differ quite a bit with regards to access (free vs. paid)

```{r, message=F, echo = FALSE, fig.height=7}
...
#  annotation_logticks(sides = "b") +
  labs(x="No. of members", y="Search term in free community name (N)", title = "Boxplot of top-23 free communities", subtitle =  "arranged by descending median number of members")
```
...
#  scale_color_manual(values = hue_pal()(23)) +
#  annotation_logticks(sides = "b") +
  labs(x="No. of members", y="Search term in paid community name (N)", title = "Boxplot of top-24 paid communities", subtitle = " arranged by descending median number of members")
```

## Price distribution box plots for different search terms

Masterclass, Mastermind and Agency have the highest median prices.
The 23 communities that include **skool** are interesting because with a median of \$57 they placed 7. But because they include four communites that cost \$500 and three times \$999 the average is the fifth highest with \$217 after (guild-\$343 (3), herd-\$299 (1), system-\$285 (15) and league-\$240 (5), which are all grouped into the *Other* category due to low sample size).
...
#### Price values (summary statistics) for all search terms
...
## Heatmap of price intervals different search terms
...
## Community names competition (which search term is in title %)

The table below shows how many results you get per search term on the discovery page. The max is 1000 hits over 34 pages. When I check that the search term actually appears in the community name I can calculate the "hit"-percentage. For **klub** I got 14 communities with 12 containing the search term in the name which makes it an 86% hit rate. When you search for \*\* business\*\* you get 1000 communities but only 51 have **business** in their name which makes it a low hit-rate of only 5.1%. **meets** (0.5%) and **workshop** (1.1%) are even lower.
...
## How often do communities have the same name
...
![14 communities named "(The) Inner Circle"](images/same_name_inner_circle.png)
:::
...
::: {.callout-note collapse="true" appearance="default"}
### Click to 12 examples of "Agency Accelerator"
![12 exmaples of agency accelerators](images/same_name_agency_accelerator.png)   
::: 


::: {.callout-note collapse="true" appearance="default"}
### Click to see examples of "The family", "The Elite" and "The Tribe"
![3 more exmaples of same name communities](images/same_name_family_tribe_elite.png)
::: 


## Communities that show up the most for different search terms

When I search based on the 44 different group names there are some communities that appear several times. Because their name either contains multiple terms, like **Elite community masterclass** or because the discovery algorithm also covers the description and about page. Around \~25% of communities only appear once in discovery. It is extremely rare to appear five or more times.
...
If you want analysis on differnt search terms, or want to learn how to do these kind of analysis, or have any other kind of data you want analyzed, send me a dm or join my free community.
